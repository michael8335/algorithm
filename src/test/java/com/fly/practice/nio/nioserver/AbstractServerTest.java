...
public class AbstractServerTest {
	...
	@BeforeClass public static void setup() throws Exception {
		server = new AbstractServer(PORT, new TwoByteMessageLength(), 512) {
			@Override protected void disconnected(SelectionKey key) {disconnected.countDown();}
			...
			@Override protected void connection(SelectionKey key) {connectKey.set(key);}
		};
	...
	@Test public void testStop() throws Exception {
		...
		AbstractServer localServer = new AbstractServer(PORT+1) {
			@Override protected void disconnected(SelectionKey key) {}
			@Override protected void messageReceived(ByteBuffer message, SelectionKey key) {}
			@Override protected void started(boolean alreadyStarted) {localServerStarted.countDown();}	    
			@Override protected void stopped() {localServerStopped.countDown();}
	...
	@Test public void testStartTwice() throws Exception {
		...
		AbstractServer localServer = new AbstractServer(PORT+1) {
			@Override protected void disconnected(SelectionKey key) {}
			@Override protected void messageReceived(ByteBuffer message, SelectionKey key) {}
			@Override protected void started(boolean alreadyStarted) {
				if (alreadyStarted) {
	...
	@Test public void testConnectionKey() throws Exception {
		messageLatch = new CountDownLatch(1);
		messageHolder.clear();
		Socket sc = new Socket("localhost", PORT); 
		OutputStream os = sc.getOutputStream();
	...
	@Test public void testSendSplitMessage() throws Exception {
		messageLatch = new CountDownLatch(1);
		messageHolder.clear();
		Socket sc = new Socket("localhost", PORT); 
		OutputStream os = sc.getOutputStream();
	...
	@Test public void testSendShortMessage() throws Exception {
		messageLatch = new CountDownLatch(1);
		messageHolder.clear();
		Socket sc = new Socket("localhost", PORT); 
		OutputStream os = sc.getOutputStream();
	...
	@Test public void testSendOverLengthMessage() throws Exception {
	...
	@Test public void testSendMultipleMessage() throws Exception {
		messageLatch = new CountDownLatch(2);
		messageHolder.clear();
		Socket sc = new Socket("localhost", PORT); 
		OutputStream os = sc.getOutputStream();
	...
	@Test public void testSendOverDefaultLengthMessage() throws Exception {
	...
	@Test public void testWriteOverDefaultLengthMessage() throws Exception {
		...
		long length = new TwoByteMessageLength().bytesToLength(new byte[] {(byte)is.read(), (byte)is.read()});
	...
	@Test public void testWrite() throws Exception {
		...
		byte[] compare = new byte[] {(byte)72, (byte)101, (byte)108, (byte)108, (byte)111, (byte)33, (byte)10};
	...
	@Test public void testWriteDisconnection() throws Exception {
		messageLatch = new CountDownLatch(1);
